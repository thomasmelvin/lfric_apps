...
contains
  ...
  subroutine horizontal_cubic_sl_metric_code( nlayers,         &
    ...
    real(kind=r_tran)   :: u, v, dzdx, dzdy, z_upwind, z_downwind
    real(kind=r_tran)   :: z_l, z_r, up, um, vp, vm

    ! Interpolation coefficients
    real(kind=r_tran), parameter :: x0 = -1.5_r_tran
    real(kind=r_tran), parameter :: x1 = -0.5_r_tran
    real(kind=r_tran), parameter :: x2 =  0.5_r_tran
    real(kind=r_tran), parameter :: x3 =  1.5_r_tran
    real(kind=r_tran), parameter :: xx = 0.0_r_tran
    real(kind=r_tran), parameter :: a0 = (xx-x1)*(xx-x2)*(xx-x3)/((x0-x1)*(x0-x2)*(x0-x3))
    real(kind=r_tran), parameter :: a1 = (xx-x0)*(xx-x2)*(xx-x3)/((x1-x0)*(x1-x2)*(x1-x3))
    real(kind=r_tran), parameter :: a2 = (xx-x0)*(xx-x1)*(xx-x3)/((x2-x0)*(x2-x1)*(x2-x3))
    real(kind=r_tran), parameter :: a3 = (xx-x0)*(xx-x1)*(xx-x2)/((x3-x0)*(x3-x1)*(x3-x2))

    real(kind=r_tran), parameter :: b0 = -1.0/6.0!(xx-x1)*(xx-x2)/((x0-x1)*(x0-x2))
    real(kind=r_tran), parameter :: b1 = 5.0/6.0!(xx-x0)*(xx-x2)/((x1-x0)*(x1-x2))
    real(kind=r_tran), parameter :: b2 = 2.0/6.0!(xx-x0)*(xx-x1)/((x2-x0)*(x2-x1))
    real(kind=r_tran), parameter :: c1 = 2.0/6.0!(xx-x2)*(xx-x3)/((x1-x2)*(x1-x3))
    real(kind=r_tran), parameter :: c2 = 5.0/6.0!(xx-x1)*(xx-x3)/((x2-x1)*(x2-x3))
    real(kind=r_tran), parameter :: c3 = -1.0/6.0!(xx-x1)*(xx-x2)/((x3-x1)*(x3-x2))
   
    if ( .false. ) then
    do k = 0, nlayers
      km = max(0, k-1)
      kp = min(nlayers-1, k)
      u =  0.25_r_tran*(wind(map_w2h(1)+km) + wind(map_w2h(3)+km) + wind(map_w2h(1)+kp) + wind(map_w2h(3)+kp))
      v = -0.25_r_tran*(wind(map_w2h(2)+km) + wind(map_w2h(4)+km) + wind(map_w2h(2)+kp) + wind(map_w2h(4)+kp))

      ! dzdx
      ! Compute upwind Z
      !if ( u > 0.0_r_tran ) then
        z_upwind = a0*z(stencil_map(1,3,1)+k) + a1*z(stencil_map(1,2,1)+k) &
                 + a2*z(stencil_map(1,1,1)+k) + a3*z(stencil_map(1,2,3)+k) 
      !else
        z_downwind = a0*z(stencil_map(1,3,3)+k) + a1*z(stencil_map(1,2,3)+k) &
                 + a2*z(stencil_map(1,1,3)+k) + a3*z(stencil_map(1,2,1)+k)
      !end if
      !dzdx = sign(1.0_r_tran, u)*(z(stencil_map(1,1,1)+k) - z_upwind)/0.5_r_tran
      dzdx = (z_downwind - z_upwind)

      ! dzdy
      ! Compute upwind Z
      if ( v > 0.0_r_tran ) then
        z_upwind = a0*z(stencil_map(1,3,2)+k) + a1*z(stencil_map(1,2,2)+k) &
                 + a2*z(stencil_map(1,1,2)+k) + a3*z(stencil_map(1,2,4)+k) 
      else
        z_upwind = a0*z(stencil_map(1,3,4)+k) + a1*z(stencil_map(1,2,4)+k) &
                 + a2*z(stencil_map(1,1,4)+k) + a3*z(stencil_map(1,2,2)+k)
      end if
      dzdy = sign(1.0_r_tran, v)*(z(stencil_map(1,1,1)+k) - z_upwind)/0.5_r_tran

      increment(map_wf(1)+k) = u*dzdx + v*dzdy
    end do
